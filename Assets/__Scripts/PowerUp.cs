...
public class PowerUp : MonoBehaviour
{
    ...
    // Необычное, но удобное применение Vector2. Х хранит минимальное значение, а Y - максимальное значение для Random.Range()
    ...
    private void Awake()
    {
        ...
        transform.rotation = Quaternion.identity; // Равноценно отсутствию поворота
    ...
    private void Update()
    {
        ...
        // Со значениями по умолчанию бонус существует 10 секунд, затем расстворяется в теч.4 сек.
        float u = (Time.time - (birthTime + lifeTime)) / fadeTime;
        // В течении lifeTime секунд значение U будет <= 0. Затем оно станет положительным,
    ...
    {
        // Получить WeaponDifinition из Main
    ...
    public void AbsorbedBy(GameObject target)
    {
        ...
        // Можно было бы реализовать эффект поглащения бонуса, уменьшая его размеры
        // с течением нескольких кадров, но пока просто уничтожим 
        Destroy(this.gameObject);
    }
}
